...
const HIDDEN_PATHS = new Set(["/login", "/registration", "/myWardrobe/1-5","/luggagePacking/selectItems","/myOutfits/uploadOutfitPic","/myOutfits/selectItems"]);
...
export default function Menu() {
	...
	return (
		<div role="tablist" className="tabs tabs-box absolute inset-x-0 bottom-0 h-[10%] w-full rounded-none bg-base-300 p-2">
			<Link
				href="/myWardrobe/1-1"
				className={`tab h-[80%] self-center flex flex-1 items-center justify-center rounded-xl border-0 text-base ${isMyWardrobe ? "bg-base-100 text-base-content font-semibold" : "text-base-content/60"}`}
			>
				我的衣櫃
			</Link>
			<Link
			...
			<Link
				href="/myAccount"
				className={`tab h-[80%] self-center flex flex-1 items-center justify-center rounded-xl border-0 text-base ${isMyAccount ? "bg-base-100 text-base-content font-semibold" : "text-base-content/60"}`}
			>
				我的帳號
			</Link>
		</div>
