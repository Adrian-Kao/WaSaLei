...
export default function CameraCapturePage() {
  ...
  async function handleUploadAndNext() {
    if (!previewUrl || !imageSize || viewportSize === 0) return;
    setUploading(true);
    setErrorMessage("");
    ...
    try {
      ...
      const croppedFile = new File([blob], "outfit-cropped.png", { type: "image/png" });

      const res = await import("@/lib/api/outfits");
      const result = await res.uploadOutfitImage(croppedFile);
      if (result && result.path) {
        // extract plain filename from path: "pictures/Outfits/prepare/UUID.ext"
        const filename = result.path.split("/").pop() ?? result.path;
        router.push(`/myOutfits/createOutfits?imageFilename=${encodeURIComponent(filename)}`);
      } else {
